...
int main(int argc, char *argv[]) {
    ...
    MPI_Scatter(&vectorInicial[ini], dimVectorLocal, MPI_DOUBLE, vectorLocal, dimVectorLocal, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    MPI_Gather(&sumaLocal, 1, MPI_DOUBLE, vectorGather, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (miId == 0) {
        ...
        // ... (I)
        printf("Tº Secuencial: %f\n", tSec);
        printf("Tº Paralelo: %f\n", tPar);
        printf("Tº Distribución por Bloques: %f\n", tDis);
        printf("Tº Incremento tSec vs tPar: %f\n", tSec / tPar);
        printf("Tº Incremento tSec vs (tDis+tPar): %f\n", tSec / (tPar + tDis));
    }
